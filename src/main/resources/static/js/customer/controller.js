...
const filter = async function () {
  try {
    // Use the API endpoint for filtering
    const filteredCars = await model.filterCars(model.state.userFilters);
    
    if (filteredCars && filteredCars.length > 0) {
      carsView.render(filteredCars, model.state.favourites, active === "favourites");
      carsView.handle();
      handleView();
      showAlert("Showing filter results");
    } else {
      carsView.render([], model.state.favourites, active === "favourites");
      carsView.handle();
      handleView();
      showAlert("No data found", false);
    }
